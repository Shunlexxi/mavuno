  ...
  const execute = async (
    ...
    switch (methodName) {
      ...
      case "hedera_executeTransaction": {
        ...
        const transactionList = transactionToBase64String(
          signedHederaTx as HederaTransaction
        );
      ...
      case "hedera_signMessage": {
        ...
        const { signatureMap } =
          await walletProvider.hedera_signMessage(signParams);

        // Decode and format the signature for display
        const { display, details } = formatSignatureDisplay(
          signatureMap,
          p.message
        );
      ...
      case "hedera_signAndExecuteTransaction": {
        ...
        const result = await walletProvider.hedera_signAndExecuteTransaction({
          signerAccountId: "hedera:testnet:" + accountId,
          transactionList: transactionToBase64String(
            transaction as HederaTransaction
          ),
        });
        sendTxId(result.transactionId);
      ...
      case "hedera_signAndExecuteQuery": {
        ...
        const { response } =
          await walletProvider.hedera_signAndExecuteQuery(queryParams);
        const accountInfo = AccountInfo.fromBytes(
