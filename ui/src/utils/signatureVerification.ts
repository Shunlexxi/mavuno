  ...
  try {
    ...
    if (sigMapObj.sigMap || sigMapObj.sigPair) {
      ...
      if (firstSig) {
        ...
        const sig =
          firstSig.ed25519 || firstSig.ECDSASecp256k1 || firstSig.signature;
        ...
        if (sig) {
          ...
          return {
            formatted: `Signature: 0x${sigHex.substring(
              0,
              20
            )}...${sigHex.substring(sigHex.length - 20)}`,
            details,
          };
        }
      }
    ...
    const sigMap = signatureMap as { signature?: string };
    if (sigMap.signature) {
      const sigHex = Buffer.from(sigMap.signature, "base64").toString("hex");
      return {
        formatted: `0x${sigHex}`,
        details: {
          hex: sigHex,
...
export async function verifySignature(
  ...
  try {
    ...
    return {
      isValid: true,
      details: `Signature format valid. Message: "${message.substring(0, 50)}${
        message.length > 50 ? "..." : ""
      }"`,
    };
  } catch (error) {
