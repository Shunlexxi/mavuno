...
import { ApiResponse, CreatePledgeRequest, UpdatePledgeRequest, PledgeFilters } from '@/types/api';
import { farmersService } from './farmersService';

// Mock data (to be removed when backend is integrated)
let mockPledges: Pledge[] = [];
let mockPledgeStats: PledgeStats = {
  totalPledged: 10000,
  activePledges: 2,
  lockedPledges: 1,
};
...
export class PledgesService {
    ...
    try {
      ...
      if (filters?.pledgerId) {
        filteredPledges = filteredPledges.filter(pledge => 
          pledge.pledgerId === filters.pledgerId
        );
      }

      if (filters?.farmerId) {
        filteredPledges = filteredPledges.filter(pledge => 
          pledge.farmerId === filters.farmerId
        );
      }

      if (filters?.status) {
        filteredPledges = filteredPledges.filter(pledge => 
          pledge.status === filters.status
        );
      }
  ...
  async getPledgeStats(pledgerId: string): Promise<ApiResponse<PledgeStats>> {
    try {
      ...
      const totalPledged = userPledges.reduce((sum, pledge) => 
        pledge.status !== 'withdrawn' ? sum + pledge.amount : sum, 0
      );
  ...
  // Create new pledge
  async createPledge(pledgeData: CreatePledgeRequest): Promise<ApiResponse<Pledge>> {
    try {
      // Simulate network delay
      await new Promise(resolve => setTimeout(resolve, 600 + Math.random() * 600));

      // Get farmer data
      const farmerResponse = await farmersService.getFarmerById(pledgeData.farmerId);
  ...
  async updatePledge(request: UpdatePledgeRequest): Promise<ApiResponse<Pledge>> {
    try {
      // Simulate network delay
      await new Promise(resolve => setTimeout(resolve, 500 + Math.random() * 500));

      const pledgeIndex = mockPledges.findIndex(p => p.id === request.pledgeId);
      if (pledgeIndex === -1) {
        return {
          data: {} as Pledge,
          success: false,
