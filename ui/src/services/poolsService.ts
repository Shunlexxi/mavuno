...
export class PoolsService {
  ...
  async performPoolAction(request: PoolActionRequest): Promise<ApiResponse<PoolPosition>> {
    try {
      // Simulate network delay
      ...
      const newPosition: PoolPosition = {
        ...
        type: request.action === 'supply' || request.action === 'withdraw' ? 'supply' : 'borrow',
  ...
  async getUserPoolPositions(userId: string): Promise<ApiResponse<PoolPosition[]>> {
    try {
      // Simulate network delay
      await new Promise(resolve => setTimeout(resolve, 250 + Math.random() * 350));

      const userPositions = mockPoolPositions.filter(pos => pos.userId === userId);
  ...
  generatePoolChartData(poolId: string, days: number = 30): { day: number; apy: number }[] {
